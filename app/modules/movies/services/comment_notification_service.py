...
class CommentNotificationService:
    ...
    async def notify_reply(self, parent_user_email: str, reply_text: str, movie_name: str):
        subject = f"You get answer on comment on movie: {movie_name}"
        body = f"Somebody answer your comment:\n\n{reply_text}"
        await self._email_sender._send_email(
            recipient=parent_user_email,
            subject=subject,
            html_content=body
        )

    async def notify_like(self, parent_user_email: str, movie_name: str):
